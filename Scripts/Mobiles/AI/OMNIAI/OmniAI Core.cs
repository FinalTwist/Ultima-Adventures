...
namespace Server.Mobiles
{
    public partial class OmniAI : BaseAI
    {
		...
		{
		    if (this.m_Mobile is BaseBlue || this.m_Mobile is BaseRed)
			return true; // BaseBlue and BaseRed are melee fighters capable of using magic skills
	    ...
	    {
		int minRange = 0;
                if (this.m_Mobile.Weapon is BaseRanged) 
		{
		    minRange = this.m_Mobile.Weapon.MaxRange - 2;
		}
		this.m_Mobile.RangeFight = Math.Max(6, minRange);

		if (!(this.m_Mobile.Weapon is BaseRanged) && (this.m_Mobile.Mana < 15 && combatant != null && combatant.Weapon is BaseRanged))
		    this.m_Mobile.RangeFight = 1;
	    }
            ...
            {
		if (this.m_Melees || m_Mobile.GetDistanceToSqrt(combatant) >= this.m_Mobile.RangeFight - 1)
		{
		...
		range = m_Mobile.RangeFight;
            ...
            else if (!this.m_Melees && m != null && (!(m.Weapon is BaseRanged) || this.m_Mobile.Mana >= 20))
            {
                if (this.m_Mobile.InRange(m, Math.Max(1, m_Mobile.RangeFight - 3)) && this.CheckMove())
                {
	...
	public static void SetRandomSkillSet(BaseCreature m, double min, double max)
	{
	    ...
	    // Reset the skills in sets so they don't get combined over time during reloads
		...
		switch (skillSet)
		{
			...
			if (m is BaseBlue || m is BaseRed)
			{
			    for (int n = 0; i < 15; i++)
				m.PackItem( new GreaterManaPotion( ) );
			}
			break;
		    case 2:
			...
			if (m is BaseBlue || m is BaseRed)
			{
			    // Bards should have an alt mode of attack, Archery is good, so why not
			    m.SetSkill(SkillName.Archery, min, max);
			    int rangedType = Utility.RandomMinMax(1, 7);
			    //if (m.Weapon != null) { m.Weapon.Delete(); }
			    switch( rangedType )
			    {
				case 1:
				    m.AddItem( new Bow() );
				...
				case 2:
				    ...
				    break;
				case 3:
				    m.AddItem( new ElvenCompositeLongbow() );
				    m.AddItem( new Arrow( Utility.Random(100, 200) ) );
				    break;
				case 4:
				    m.AddItem( new MagicalShortbow() );
				    m.AddItem( new Arrow( Utility.Random(100, 200) ) );
				    break;
				case 5:
