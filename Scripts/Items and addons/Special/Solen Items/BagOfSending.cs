		...
		private class SendTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				else if ( targeted is Item )
				{
					...
					else if ( item.Parent is CoinPouch || item.Parent is BagOfHolding || item.Parent is LumberjackPouch || item.Parent is MinersPouch || item.Parent is SmallBagofHolding )
					{
						from.SendMessage( "Some magic is affecting the weight of this item." );
					}
