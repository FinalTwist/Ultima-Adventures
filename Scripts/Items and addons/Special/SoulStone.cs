...
namespace Server.Items
{
	public class SoulStone : Item, ISecurable
	{
		...
		protected virtual bool CheckUse( Mobile from )
		{
			...
			if (pm.SoulBound)
			{
				from.SendMessage("The Item doesn't seem to work for you.");
				return false;
			}
		...
		private class ConfirmTransferGump : Gump
		{
			...
			public override void OnResponse( NetState sender, RelayInfo info )
			{
				...
				if ( m_Stone.Account != null &&  from.Account.Username != m_Stone.Account )
				{
					if (Utility.RandomDouble() > 0.75 )
					{
						from.SendMessage( "his is an Account Bound Soulstone, and your character is not bound to it.  The stone lost a small portion of its power when you used it." ); // T  You cannot use this Soulstone.
						int scalar = Utility.RandomMinMax(95, 100);
						skillValue *= (double)scalar/100;
						if (fromSkill.Base > skillValue)
						{
							if (Utility.RandomBool())
								from.SendMessage("The stone corrupted your knowledge of this skill!");
							else
							{
								from.SendMessage("You feel a strange energy from the stone and immediately stop touching it!");
								return;
							}
						}
