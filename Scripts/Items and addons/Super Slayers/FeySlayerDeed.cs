	...
	public class FeySlayerTarget : Target
	{
		...
		protected override void OnTarget( Mobile from, object target )
		{
						...
						if ( target is BaseInstrument )
			{
				...
				if ( ((BaseInstrument)item).Slayer == SlayerName.Repond || ((BaseInstrument)item).Slayer2 == SlayerName.Repond )
				{
				...
				else
				{
					...
					{
						if( ((BaseInstrument)item).Slayer != SlayerName.None && ((BaseInstrument)item).Slayer2 != SlayerName.None )
						{
							from.SendMessage( "Your instrument already has two slayers! One must be removed first." );
						}
						
						if( ((BaseInstrument)item).Slayer == SlayerName.None )
						{
							((BaseInstrument)item).Slayer = SlayerName.Repond;
							from.SendMessage( "Your instrument magically becomes a repond slayer..." );
							m_Deed.Delete();
						}
						else if( ((BaseInstrument)item).Slayer2 == SlayerName.None )
						{
							((BaseInstrument)item).Slayer2 = SlayerName.Repond;
							from.SendMessage( "Your instrument magically becomes a repond slayer..." );
							m_Deed.Delete();
						}
					}
