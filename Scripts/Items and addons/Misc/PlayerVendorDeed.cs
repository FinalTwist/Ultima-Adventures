	...
	public class ContractOfEmployment : Item
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else
			{
				...
				if ( house == null && !MyServerSettings.AllowFreePVPlacement() )
				{
					from.SendLocalizedMessage( 503240 ); // Vendors can only be placed in houses.	
				}
				else if ( !BaseHouse.NewVendorSystem && house != null && !house.IsFriend( from ) )
				{
				...
				else if ( BaseHouse.NewVendorSystem && house != null && !house.IsOwner( from ) )
				{
				...
				else if ( house != null && !house.Public && !house.CanPlaceNewVendor() ) 
				{
				...
				else
				{
					...
					{
						Mobile v = new PlayerVendor( from, house != null ? house : BaseHouse.FindHouseAt(from) );
