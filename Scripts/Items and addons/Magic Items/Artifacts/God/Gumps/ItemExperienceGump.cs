...
namespace Server.Gumps
{
    ...
    public class ItemExperienceGump : Gump
	{
		...
		private ItemExperienceGump( Mobile from, Item item, AttributeCategory cat, GumpPage page ) : base( 40, 40 )
		{
			...
			if (m_Item is BaseQuiver) // remove Button Cannot add ressit to it!
			{ }
			else
		...
		public void CreateAttributeList( AttributeCategory cat )
		{
			...
			for ( int i = 0; i < LevelAttributes.m_Attributes.Length; ++i )
			{
				if (LevelAttributes.m_Attributes[i].m_Category == cat)
				{
                    ...
                    if (m_Item is BaseWeapon)
						...
						attrvalue = ( ( BaseQuiver )m_Item ).Attributes[ LevelAttributes.m_Attributes[ i ].m_Attribute ];
					else
                        return;
			...
			/*else if (m_Item is BaseQuiver)
			{
				if (cat == AttributeCategory.Misc)
				{
					for (int i = 0; i < LevelAttributes.m_QuiverAttributes.Length; ++i)
					{
						pageindex = index % 10;
						...
						attrvalue = ( ( BaseQuiver )m_Item ) LevelAttributes.m_QuiverAttributes[ i ].m_Attribute;

						if (attrvalue < LevelAttributes.m_QuiverAttributes[ i ].m_MaxValue)
							AddButton( 301, 116 + ( pageindex * 20 ), 4005, 4007, GetButtonID( 6, i ), GumpButtonType.Reply, 0 );
						AddLabel( 337, 117 + ( pageindex * 20 ), LabelHue, LevelAttributes.m_QuiverAttributes[ i ].m_Name + " (" + GetPointCost( m_Item, LevelAttributes.m_QuiverAttributes[ i ].m_XP ) + "sp)" );
						AddLabel( 540, 117 + ( pageindex * 20 ), LabelHue, attrvalue.ToString() );
		...
		public static int GetPointCost(Item m_Item, int xp)
		{
            ...
            if (LevelItems.DoubleArtifactCost)
			{
				...
				{
					if (( ( BaseQuiver )m_Item ).ArtifactRarity > 0)
						cost *= 2;
				}
				else
				{
				...
				case 2: // Attribute selected
				{
                    ...
                    if ((levitem.Points - cost) >= 0)
                    {
                        ...
                        if (index >= 0 && index < LevelAttributes.m_Attributes.Length)
                        {
							...
							{
								attrvalue = ( ( BaseQuiver )m_Item ).Attributes[ LevelAttributes.m_Attributes[ index ].m_Attribute ];
								if (attrvalue < LevelAttributes.m_Attributes[ index ].m_MaxValue)
								{
									( ( BaseQuiver )m_Item ).Attributes[ LevelAttributes.m_Attributes[ index ].m_Attribute ] += 1;
									levitem.Points -= cost;
								}
							}
							else
