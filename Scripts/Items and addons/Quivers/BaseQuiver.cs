...
namespace Server.Items
{
	...
	public class BaseQuiver : Container, ICraftable
	{
		...
		*/
		...
		{
			typeof( Arrow ), typeof( Bolt ), typeof( MageEye ),typeof( Krystal ),typeof( HarpoonRope ), typeof (ThrowingWeapon)
        };
		...
		public override bool CheckHold( Mobile m, Item item, bool message, bool checkItems, int plusItems, int plusWeight )
		{
            ...
            Item ThrowingWeapon = this.FindItemByType(typeof(ThrowingWeapon));
            ...
            if (ThrowingWeapon != null && item is ThrowingWeapon)
            {
                if (ThrowingWeapon.Amount + item.Amount <= m_Capacity)
                    return true;
            }
		...
		public override void GetProperties( ObjectPropertyList list )
		{
			...
			if ( ammo != null )
			{
                ...
                if (ammo is MageEye)
                    list.Add(1116651, "{0}\t{1}", ammo.Amount, Capacity); // Ammo: ~1_QUANTITY~/~2_CAPACITY~ arrows
                if (ammo is Krystal)
                    list.Add(1075265, "{0}\t{1}", ammo.Amount, Capacity); // Ammo: ~1_QUANTITY~/~2_CAPACITY~ arrows
                if (ammo is HarpoonRope)
                    list.Add(1075265, "{0}\t{1}", ammo.Amount, Capacity); // Ammo: ~1_QUANTITY~/~2_CAPACITY~ arrows
                 if (ammo is ThrowingWeapon)
                     list.Add(1044117, "{0}\t{1}", ammo.Amount, Capacity); // Ammo: ~1_QUANTITY~/~2_CAPACITY~ arrows
			...
			*/
			...
			if (( prop = ( m_Attributes.WeaponDamage ) ) != 0 && !md)
				list.Add( 1060401, prop.ToString() ); // damage increase ~1_val~%

			if ( (prop = m_Attributes.DefendChance) != 0 && !md)
				list.Add( 1060408, prop.ToString() ); // defense chance increase ~1_val~%

			if (( prop = m_DamageIncrease ) != 0 && !md)
				list.Add( 1074762, prop.ToString() ); // Damage modifier: ~1_PERCENT~%
