...
namespace Server.Items
{
	public class CraftDoor : Item
	{
		...
		[Constructable]
		public CraftDoor() : this( new Point3D( 0, 0, 0 ), null, false )
		{
		}

		[Constructable]
		public CraftDoor( Point3D pointDest, Map mapDest ) : this( pointDest, mapDest, false )
		{
		}

		[Constructable]
		public CraftDoor( Point3D pointDest, Map mapDest, bool creatures ) : base( 0x6C5 )
		{
			Movable = false;
			Visible = true;
		...
		public override void OnDoubleClick( Mobile m )
		{
			if ( m.InRange( this.GetWorldLocation(), 2 ) )
			{
				if ( m_Active && m is PlayerMobile )
				{
					...
					if ( pm.NpcGuild == NpcGuild.AlchemistsGuild || pm.NpcGuild == NpcGuild.MinersGuild || pm.NpcGuild == NpcGuild.TinkersGuild || pm.NpcGuild == NpcGuild.TailorsGuild || pm.NpcGuild == NpcGuild.BlacksmithsGuild || pm.NpcGuild == NpcGuild.CarpentersGuild || pm.NpcGuild == NpcGuild.LibrariansGuild ) 
						StartTeleport( m );
					else 
						m.SendMessage(0,  "You need to be a member of a crafting guild to enter." );
				}
			}
			else
			{
		...
		public CraftDoor( Serial serial ) : base( serial )
		{
		}
