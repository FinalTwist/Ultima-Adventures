	...
	public class Confidence : SamuraiSpell
	{
		...
		public static void BeginConfidence( Mobile m )
		{
			...
			double bushido = m.Skills[SkillName.Bushido].Value;
            		BuffInfo.AddBuff(m, new BuffInfo(BuffIcon.Confidence, 1060596, 1153809, TimeSpan.FromSeconds(4), m, string.Format("{0}\t{1}\t{2}", ((int)(bushido / 12)).ToString(), ((int)(bushido / 5)).ToString(), "100"))); 
