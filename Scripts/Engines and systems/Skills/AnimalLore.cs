...
namespace Server.SkillHandlers
{
    ...
    public class AnimalLoreGump : Gump
    {
        ...
        public AnimalLoreGump(BaseCreature c, int source, AnimalLoreGumpPage initialPage = AnimalLoreGumpPage.Attributes) : base(25, 25)
        {
            m_source = source;
            ...
            var pageValues = new[]
            {
                AnimalLoreGumpPage.Attributes,
                AnimalLoreGumpPage.Resistances,
                AnimalLoreGumpPage.Damage,
                AnimalLoreGumpPage.Skills,
                AnimalLoreGumpPage.JakoTamingSkills,
                AnimalLoreGumpPage.JakoTamingAttributes,
                AnimalLoreGumpPage.Misc,
            };
            
            int pages = 5;
            if (c.Tamable) pages++;
            if (c.Tamable && c.Controlled && c.ControlMaster != null) pages++;
            
            var currentPageIndex = 1;
            AddPage(currentPageIndex);
            for (int i = 0; i < pageValues.Length; i++)
            {
                var pagePosition = (AnimalLoreGumpPage)(((int)initialPage + i) % pageValues.Length);
                if (!AddAnimalLorePage(c, pagePosition, source)) continue;

                var previousPage = 1 < currentPageIndex ? currentPageIndex - 1 : pages;
                var nextPage = pages <= currentPageIndex ? 0 : currentPageIndex + 1;
                AddButton(317, 358, 5603, 5607, 0, GumpButtonType.Page, previousPage);
                AddButton(340, 358, 5601, 5605, 0, GumpButtonType.Page, nextPage);
        ...
        private bool AddAnimalLorePage(BaseCreature c, AnimalLoreGumpPage page, int source)
        {
            switch (page)
            {
                case AnimalLoreGumpPage.Attributes:
                    {
                ...
                case AnimalLoreGumpPage.Resistances:
                    {
                        if (Core.AOS)
                        {
                            AddImage(128, 152, 2086);
                ...
                case AnimalLoreGumpPage.Damage:
                    {
                        if (Core.AOS)
                        {
                            AddImage(128, 152, 2086);
                ...
                case AnimalLoreGumpPage.Skills:
                    {
                        AddImage(128, 152, 2086);
                ...
                case AnimalLoreGumpPage.JakoTamingSkills:
                    {
                        if (c.Tamable)
                        {
                            ...
                            if (c.Controlled && c.ControlMaster != null)
                            {
                                ...
                                AddHtml(275, 240, 80, 18, FormatStat((int)c.Experience), false, false);

                                AddHtml(153, 258, 130, 18, "<basefont color=#4A3929>Exp Needed</basefont>", false, false);
                                AddHtml(275, 258, 80, 18, FormatStat((int)c.ExpToNextLevel), false, false);
                                return true;
                            }
                            #endregion
                        }
                ...
                case AnimalLoreGumpPage.JakoTamingAttributes:
                    {
                        if (c.Tamable)
                        {
                            ...
                            if (c.ControlMaster != null && (source == 0 || source == 1))
                            {
                                Int32 locked = 0x82C;
                                Int32 up = 0x983;
                ...
                case AnimalLoreGumpPage.Misc:
                    {
                        ...
                        int? packInstinct = null;
                        string packInstinctString = null;
                        ...
                        {
                            // Instincts w/o cliloc
                            if ((c.PackInstinct & PackInstinct.Reptile) != 0) packInstinctString = "Reptile";
                            else packInstinct = 3000340;
                        }

                        if (packInstinct.HasValue) AddHtmlLocalized(153, 204, 160, 18, packInstinct.Value, LabelColor, false, false);
                        else AddHtml(153, 204, 160, 18, packInstinctString, false, false);

                        if (c.Tamable)
                        {
                            AddImage(128, 221, 2086);
                            AddHtmlLocalized(147, 221, 160, 18, 1112656, 200, false, false); // Control

                            AddHtml(153, 239, 160, 18, "<basefont color=#4A3929>Taming Required</basefont>", false, false);
                            AddHtml(280, 239, 75, 18, FormatDouble(c.MinTameSkill), false, false);

                            AddHtml(153, 257, 160, 18, "<basefont color=#4A3929>Control Slots</basefont>", false, false);
                            AddHtml(280, 257, 75, 18, FormatStat(c.ControlSlots), false, false);
                        }
        ...
        public override void OnResponse(Server.Network.NetState sender, RelayInfo info)
        {
            ...
            {
                case 1001: success = m_bc.m_jakoAttributes.GetAttribute(JakoAttributesEnum.Hits).DoOnClick(m_bc, out reply); break;
                case 1002: success = m_bc.m_jakoAttributes.GetAttribute(JakoAttributesEnum.Stam).DoOnClick(m_bc, out reply); break;
                case 1003: success = m_bc.m_jakoAttributes.GetAttribute(JakoAttributesEnum.Mana).DoOnClick(m_bc, out reply); break;
                case 1004: success = m_bc.m_jakoAttributes.GetAttribute(JakoAttributesEnum.BonusPhysResist).DoOnClick(m_bc, out reply); break;
                case 1005: success = m_bc.m_jakoAttributes.GetAttribute(JakoAttributesEnum.BonusFireResist).DoOnClick(m_bc, out reply); break;
                case 1006: success = m_bc.m_jakoAttributes.GetAttribute(JakoAttributesEnum.BonusColdResist).DoOnClick(m_bc, out reply); break;
                case 1007: success = m_bc.m_jakoAttributes.GetAttribute(JakoAttributesEnum.BonusPoisResist).DoOnClick(m_bc, out reply); break;
                case 1008: success = m_bc.m_jakoAttributes.GetAttribute(JakoAttributesEnum.BonusEnerResist).DoOnClick(m_bc, out reply); break;
            }
            ...
            if (!string.IsNullOrWhiteSpace(reply))
            {
                ...
                from.CloseGump(typeof(AnimalLoreGump));
                if (success) from.SendGump(new AnimalLoreGump(m_bc, m_source, AnimalLoreGumpPage.JakoTamingAttributes)); // Reopen the gump after spending points
            }
            else
            {
