...
namespace Knives.TownHouses
{
	...
	public class TownHouseSign : Item
	{
		...
		public virtual void ClearHouse()
		{
			...
			if (c_House.DecayLevel != DecayLevel.Collapsed)
				PackUpItems(); // do not pack up items if the house is collapsed!
			else 
				ReleaseItems();
		...
		protected void PackUpHouse()
		{
			...
			Mobile from = c_House.Owner;
			int cost = c_House.Price;
							...
							else if (chance <= .98)
							{
								...
								cost = (int)((double)cost * 1.15);
							}
						

			c_House.Owner.BankBox.DropItem( new BankCheck( (int)((double)cost) ) ); // final, not full refund
