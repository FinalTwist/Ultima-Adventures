...
{
    public class ClothingItemProps
    {
		#region clothing
        public static string GetClothingItemValue(Item item)
        {
				bool useartifactrarity = false;
		
				BaseClothing bc = item as BaseClothing;
				int values = CheckClothing(bc); 
				int rarityvalues = CheckArtifactClothing(bc);
		...
		public static int CheckArtifactClothing(BaseClothing item)
		{
				int rarityvalue = 0;
        ...
        {
            return (string)(GetClothingItemValue(item) + orig + "<BASEFONT COLOR=#FFFFFF>");
        }
