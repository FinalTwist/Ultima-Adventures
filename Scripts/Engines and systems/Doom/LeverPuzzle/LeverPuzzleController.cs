...
namespace Server.Engines.Doom
{
	public class LeverPuzzleController : Item
	{
		...
		{
			foreach ( Item item in Map.Ilshenar.GetItemsInRange( lp_Center, 0 ) )
			{
			...
			new LeverPuzzleController().MoveToWorld( lp_Center, Map.Ilshenar );
		...
		public static Item AddLeverPuzzlePart( int[] Loc, Item newitem  )
		{
			...
			{
				newitem.MoveToWorld( new Point3D(Loc[0], Loc[1], Loc[2]), Map.Ilshenar );
			}
			return newitem;
		}
			...
			else
			{
				if( TheirKey == MyKey )
				{
					...
					if (( m_Successful = ( m_Player=GetOccupant( 0 ))) != null )
					{
						...
						m_Player.MoveToWorld( lr_Enter, Map.Ilshenar );
		...
		public class RockTimer : Timer
		{
			...
			{
				if( m_Player == null || !(m_Player.Map == Map.Ilshenar) )
				{
					Stop();
				}
				else
		...
		public static void MoveMobileOut( Mobile m )
		{
			if ( m != null )
			{
				if( m is PlayerMobile && !m.Alive )
				{
					...
					{
						m.Corpse.MoveToWorld( lr_Exit, Map.Ilshenar );
					}
				}
				BaseCreature.TeleportPets( m, lr_Exit, Map.Ilshenar );
				m.Location = lr_Exit;
				m.ProcessDelta();
			}
		}
		...
		public static void PlaySounds( Point3D location, int[] sounds )
		{
			foreach( int soundid in sounds )
				Effects.PlaySound( location, Map.Ilshenar, soundid );
		}
		...
		{
			Effects.SendPacket( p, Map.Ilshenar, new LocationEffect( p, itemID, speed, duration, hue, 0 ) );
		}
		...
		{ 
			...
			new int[]{1436, 1088, 5},					/* 3D Coords for levers */
			new int[]{1443, 1082, 5}, 
			new int[]{1452, 1087, 5}, //final 1120 1024 offset
			new int[]{1443, 1095, 5},

			new int[]{1444, 1088},					/* 2D Coords for standing regions */
			new int[]{1436, 1089}, 
			new int[]{1444, 1082}, 
			new int[]{1452, 1088}, 
			new int[]{1443, 1096},

			new int[]{1588, 1116, -1}, new int[]{0x181D, 0x482}, 	/* 3D coord, itemid+hue for L.R. teles */
			new int[]{1589, 1116, -1}, new int[]{0x1821, 0x3fd}, 
			new int[]{1590, 1116, -1}, new int[]{0x1825, 0x66d}, 

			new int[]{1439, 1094, 18}, new int[]{0x12d8}, 		/* 3D coord, itemid for statues */
			new int[]{1449, 1084, 18}, new int[]{0x12d9}, 

			new int[]{1589, 1120, 6}					/* 3D Coords for Fake Box */
		};
		...
		public static Point3D lr_Exit = new Point3D( 1473, 1196, -1 );
		public static Point3D lr_Enter = new Point3D( 1587, 1120, -1 );

			/* "Center" location in puzzle */

		public static Point3D lp_Center = new Point3D( 1444, 1088, -1 );

			/* Lamp Room Area */

		public static Rectangle2D lr_Rect = new Rectangle2D( 1585, 1116, 10, 10 );
