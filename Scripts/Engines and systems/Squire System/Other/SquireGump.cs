...
namespace Server.Gumps
{
    public class SquireCustomizeGump : Gump
    {
        ...
        public SquireCustomizeGump(Squire squire) : base(50, 50)
        {
            ...
            AddBackground(0, 0, 370, 370, 9250);
    ...
    public class SquireLoreGump : Gump
    {
        ...
        private void AddStatsOption(int columnNumber, int rowNumber, bool isEnabled, int buttonType, int buttonIndexBase, string label)
        {
            const int iconWidth = 25;
            const int trueIcon = 0xD3;
            const int falseIcon = 0xD2;

            const int labelWidth = 160;
            const int labelHeight = 18;

            const int rowHeight = 22;
            const int rowHeightButton = rowHeight;
            const int baseRowY = 30;
            const int baseRowButtonY = baseRowY;

            const int columnWidth = 205;
            int x = columnNumber == 1 ? 20 : columnWidth * (columnNumber - 1);

            // Add arrow
            var icon = isEnabled ? trueIcon : falseIcon;
            var buttonIndex = isEnabled ? buttonIndexBase : buttonIndexBase + 1; // This is a bit magical, but it is what it is...
            AddButton(x, baseRowButtonY + rowHeightButton * rowNumber, icon, icon, GetButtonID(buttonType, buttonIndex), GumpButtonType.Reply, 0);

            // Add Label
            AddHtml(x + iconWidth, baseRowY + rowNumber * rowHeight, labelWidth, labelHeight, label, false, false);
        }
    }
}