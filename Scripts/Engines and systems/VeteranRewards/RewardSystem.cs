...
/*						new RewardEntry( cloaksAndRobes, 1049732, typeof( RewardRobe ), JetBlack, 1049762 ),
		...
		private static void EventSink_Login( LoginEventArgs e )
		{
			...
			if ( Core.ML && e.Mobile is PlayerMobile && !((PlayerMobile)e.Mobile).GetFlag( PlayerFlag.HasStatReward ) && HasHalfLevel( e.Mobile ) )
			{
				((PlayerMobile)e.Mobile).SetFlag(PlayerFlag.HasStatReward, true);
				e.Mobile.StatCap += 5;
			}
