    ...
    public class CockFightingControlStone : Item
    {
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            else //what to do if account not been tagged
            {
                ...
                {
		    int GoldAmount = from.Backpack.GetAmount(typeof(Gold));

                    if (GoldAmount >= m_Bet)
                    {
			from.Backpack.ConsumeTotal(typeof(Gold), m_Bet);
                        JoinGame(from, this);
                        from.SendMessage("{0} gold has been withdrawn from your backpack.  The chicken you are betting on has been placed in the arena to warm up for his fight.", m_Bet);
